...
#endif
/*
huffman编码的原理:1、最优二叉树 2、根节点到一个叶子节点的路径 不会是 根节点到另一个叶子节点的路径的前缀(如果是别人的前缀就肯定不是叶子节点)
...
所有节点放在小根堆中，取出两个节点，生成这两个节点的父节点，父节点的频率为两个子节点之和
，把父节点放到小根堆中。
重复上面步奏直到小根堆只有一个节点，那个节点就是huffman数的root节点
*/
