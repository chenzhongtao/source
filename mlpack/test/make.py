...
def make():
    ...
    for filename in file_list:
        if filename.endswith(".c") or filename.endswith(".cpp"):
            ...
            cmd = "g++ -g -O0 -I /usr/include/libxml2  %s -o %s -lmlpack" %(filename, target)
            #os.system(cmd)
...
def clean():
    ...
    file_list = os.listdir("./")
    for filename in file_list:
        if ".c" not in filename and ".py" not in filename and ".cpp" not in filename and ".txt" not in filename:
