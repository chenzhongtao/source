...
#define STARTING_EVENT_THREADS                10
#define INTERVAL_NUM 1000

#define MY_SOMAXCONN	12800 //改完没效果

//这里只是演示 如何在epoll中放私有数据，fd和inx其实在两个handler函数中都有传入
struct private_data{
    int fd;
    int idx;
};  //不要忘记加分号
    ...
    while (1)  
              ...
              if (done)  
                { 
                  ...
                  event_unregister_close(pool, fd , idx);
                }  
         return ret;
}  
