...
static void usage(void)
{
    printf("optional arguments:\n"
           ...
           "-t int,  thread num \n"
           "-l int,  loop num \n"
           "-c bool, use calloc instead of malloc \n");
}
...
void *test_fun(void *arg)
{
    ...
    for(i=0; i<block_num; i++)
    {
            if (use_calloc ==0)
            {
                mem_array[i] = (char *)MALLOC(block_size);// malloc分配内存只是现在虚拟空间上分配，只是物理空间是没有被占用的(好像会占用4K)
            }
            else
            {
