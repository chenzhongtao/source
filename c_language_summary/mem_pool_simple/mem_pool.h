...
static inline void MY_INIT_LIST_HEAD(struct my_list_head *list)
{
	list->next = NULL;
}

static inline void my_list_add(struct my_list_head *new, struct my_list_head *head)
{
    do{
    new->next = head->next;
    }while(!CAS(&head->next, new->next, new));
}

static inline struct my_list_head * list_del_and_get(struct my_list_head *head)
{
    struct my_list_head *tmp;
    
    do {
        tmp = head->next;
        if (!tmp) return NULL;
    }while(!CAS(&head->next, tmp, tmp->next));
    tmp->next = NULL;
    return tmp;
...
struct mem_pool {
        struct my_list_head  list; //内存池中未使用内存块链表 
        ...
        char             *name; //内存池名字
        struct my_list_head  global_list; //用来挂在全局内存池链表 THIS->ctx->mempool_list 上
};
